...
- **No deep relative imports across packages.** Within a package, `../foo.js` is fine. Reaching into another workspace package via relative paths is wrong — import the public entry instead (`@jentic/api-scorecard-html`). `import/no-extraneous-dependencies` blocks importing devDeps from non-test code.
