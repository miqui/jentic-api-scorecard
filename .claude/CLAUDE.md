...
- **`skills/`** — the canonical agent-skill source (distinct from the harness's `.claude/skills/`). `skills/jentic-api-scorecard/SKILL.md` (+ `references/llm-analysis.md`) teaches coding agents to use the CLI; it's the single source for three distribution paths — [TanStack Intent](https://tanstack.com/intent) (`npx @tanstack/intent install`, discovers it from the installed npm package via the `tanstack-intent` keyword + `skills/` `files` entry on `packages/cli/package.json`), the [Vercel `skills` CLI](https://github.com/vercel-labs/skills) (`npx skills add jentic/jentic-api-scorecard --skill jentic-api-scorecard`, reads it from the repo), and the [Claude Code plugin marketplace](https://docs.claude.com/en/docs/claude-code/plugins) (`/plugin marketplace add jentic/jentic-api-scorecard` → `/plugin install api-scorecard@jentic-api-scorecard`). The tree is `copyfiles`-copied into `packages/cli/` at `prepack` and `rimraf`'d at `postpack` (same pattern as `LICENSE`/`NOTICE`/`README.md`), so git keeps one copy and the tarball carries a real one. There is no skill-validation tooling or CI gate — the `tanstack-intent` keyword + shipped `skills/` tree are all Intent needs to discover the skill from a consumer's `node_modules`. Keep `SKILL.md`'s flag/exit-code tables in sync with the CLI surface (see `cli-readme-sync.md`).
- **`.claude-plugin/`** — the Claude Code plugin marketplace catalog (`marketplace.json`). Lists one plugin with `source: "./"` (repo root is the plugin root), `strict: false`, and an explicit `skills: ["./skills/jentic-api-scorecard"]` array — so there's no `plugin.json` (under `strict: false` the entry IS the whole definition; a component-declaring `plugin.json` would conflict) and no auto-discovery reliance. Reads the canonical repo-root `skills/` tree directly (no copy step — distinct from the npm tarball path). Self-serve/pull-based: no central registry, no submission. The dev harness under `.claude/` is not referenced and never leaks to installers. Growth: a second *skill* is one more `skills[]` entry; a second *plugin* gets its own `marketplace.json` entry (and a subdirectory `source` if it needs its own layout).
- **`action.yml` + `action/`** — the Marketplace-listable composite GitHub Action (Phase 19). `action.yml` at the repo root (Marketplace requires the manifest at root) wraps `npx @jentic/api-scorecard-cli@<cli-version>` for one `score --format json --detail diagnostics -o report.json` pass, then runs `action/postprocess.mjs` to derive SARIF, the HTML scorecard, and the Markdown run summary from that single capture (score-once, format-many — each formatter is a pure function of the result). The helper computes the gate (`min-score` strict `<`, `max-errors`/`max-warnings` against the **full** diagnostics, not the severity-filtered SARIF), applies the `severity` filter then the `max-findings` cap (lowest-severity-first, logged), and writes `$GITHUB_STEP_SUMMARY`; the gate verdict goes to `$GITHUB_OUTPUT` and a final step fails the build **after** the publish steps so SARIF / HTML / summary land even on failure. The SARIF upload is guarded so a fork PR's read-only token skips it with a notice instead of hard-failing. The helper imports the CLI's `./formatters/sarif` / `./formatters/markdown` exports and formatter-html's `format()`; a setup step `npm install`s them adjacent to the helper unless already resolvable (the in-repo `.github/workflows/action-selftest.yml` builds the workspace first, so the helper resolves the local build's not-yet-published exports). `cli-version` is left blank by default and resolved at runtime from the action's own `packages/cli/package.json` (which lerna bumps every release), so it tracks the action ref the consumer pinned with no manual edit — no release-time bump needed. Pre-bundling the helper is infeasible — HTML `format()` reads a sibling `dist/app/index.html` at runtime.
