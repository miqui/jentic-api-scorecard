...
- **`skills/`** — the canonical agent-skill source (distinct from the harness's `.claude/skills/`). `skills/jentic-api-scorecard/SKILL.md` (+ `references/llm-analysis.md`) teaches coding agents to use the CLI; it's the single source for both distribution paths — [TanStack Intent](https://tanstack.com/intent) (`npx @tanstack/intent install`, discovers it from the installed npm package via the `tanstack-intent` keyword + `skills/` `files` entry on `packages/cli/package.json`) and the [Vercel `skills` CLI](https://github.com/vercel-labs/skills) (`npx skills add jentic/jentic-api-scorecard --skill jentic-api-scorecard`, reads it from the repo). The tree is `copyfiles`-copied into `packages/cli/` at `prepack` and `rimraf`'d at `postpack` (same pattern as `LICENSE`/`NOTICE`/`README.md`), so git keeps one copy and the tarball carries a real one. There is no skill-validation tooling or CI gate — the `tanstack-intent` keyword + shipped `skills/` tree are all Intent needs to discover the skill from a consumer's `node_modules`. Keep `SKILL.md`'s flag/exit-code tables in sync with the CLI surface (see `cli-readme-sync.md`).
