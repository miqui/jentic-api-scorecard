...
F3. **Only when benchmark metrics were requested** (same gate as F2; skip otherwise): with a single `jq` call read `.summary.{score,level,grade}` from the baseline `./.jentic-improve-work/scorecard.json` (before) and the final `./.jentic-improve-work/score-iter-N.json` (after; baseline when none improved), plus `iterationsRun` = count of `score-iter-*.json` files, into `./.jentic-improve-work/benchmark-summary.json`; then `cp -- ./.jentic-improve-work/benchmark-summary.json "<OUT_DIR>/benchmark-summary.json"` (separate Bash call). `null` for any unavailable value — never fabricate. See "Output Files" for the shape.
