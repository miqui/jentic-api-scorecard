...
When the loop is done, place the outputs flat in `<OUT_DIR>` (the literal Output directory from the brief), each step a SEPARATE Bash/Write call. Step F2 runs only when the brief's "Report engine token usage" is "yes":
