...
- **`skills/` is the single canonical source for the agent skill**, consumed by two distribution paths that share the same `skills/<name>/SKILL.md` + `name`/`description`-frontmatter convention: [TanStack Intent](https://tanstack.com/intent) (`npx @tanstack/intent install`, which discovers the skill from the installed npm package — recognized via the `tanstack-intent` keyword and `skills/` `files` entry on `packages/cli/package.json`) and the [Vercel `skills` CLI](https://github.com/vercel-labs/skills) (`npx skills add jentic/jentic-api-scorecard --skill jentic-api-scorecard`, which reads the skill straight from this repo). The repo-root `skills/` tree is copied into `packages/cli/` at `prepack` (alongside `LICENSE`/`NOTICE`/`README.md`, via `copyfiles`) and removed at `postpack`, so the published CLI tarball carries a real copy while git keeps exactly one canonical source. `intent validate` (run as `skills:validate`) gates the skill frontmatter on PRs.
