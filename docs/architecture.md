...
- On `--with-llm` runs the engine also adds a top-level `tokenUsage` object (`inputTokens`, `outputTokens`, `totalTokens`, `llmCalls`, `model`, `provider`) reporting the LLM tokens the *scoring engine* consumed (the runner sets `report_token_usage` — see §6). It is absent without `--with-llm`. The CLI's `--detail` filter preserves unknown top-level keys (it destructures only `summary`/`details`/`diagnostics` and re-spreads the rest), so `tokenUsage` survives to `--format json` output at every detail level.
